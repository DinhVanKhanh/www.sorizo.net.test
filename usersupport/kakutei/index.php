<?php
    ...
    // ↓↓　<2021/06/16> <YenNhi> <write log to table kakutei_download>
        ...
        $DownloadProgramVersion = 'V21-00-00';
        // 計算用に[$DownloadProgramYear]をint型に変換 Kentaro.Watanabe 2022/02/11
        $DownloadExeFile = $GLOBALS['PRG_DOWNLOAD_SERVER_AWS'] . "kakutei/WeKakutei".((int)$DownloadProgramYear+1) .".exe";
    // ↑↑　<2021/06/16> <YenNhi> <write log to table kakutei_download>

    if (isset($_REQUEST['option'])) {
        if ($_REQUEST['option'] == 'manual') {
            execDownload("\downloadmanual_log_".$DownloadProgramYear .".txt", ",みんなの確定申告". $DownloadProgramYear.",マニュアル,Ver210125,", "download_files/sksmanualKojin".((int)$DownloadProgramYear+1) .".pdf", true);
        }
    // ↓↓　<2021/06/16> <YenNhi> <write log to table kakutei_download>
        // execDownload("\download_log_kakutei_2020.txt", ",みんなの確定申告2020(V20-00-00),プログラム,Ver210125,", $GLOBALS['PRG_DOWNLOAD_SERVER_AWS'] . "kakutei/WeKakutei2022.exe", true)
       execDownloadAndSaveDB($DownloadProgramName, $DownloadProgramYear, $DownloadProgramVersion, $GLOBALS['PRG_DOWNLOAD_SERVER_AWS'] . "kakutei/WeKakutei".((int)$DownloadProgramYear+1) .".exe");
    // ↑↑　<2021/06/16> <YenNhi> <write log to table kakutei_download>
    }
            ...
            <section id="contents" class="service">
                <div class="box-contents">
                    <hgroup>
                        ...
                        <dl class="clearfix">
                            <dt><span><a href="#downloadbox" onfocus="this.blur()"><img src="images/logo_kakutei<?= $DownloadProgramYear ?>_w500.png" alt="みんなの確定申告"></a></span></dt>
                    ...
                    <p>昨年の〈<?= getEra($DownloadProgramYear-1) ?>申告用〉で入力された申告者データ（氏名や住所など）を、本年の〈<?= getEra($DownloadProgramYear) ?>申告用〉へ引き継ぐことができます。データを引き継ぐ場合は、昨年の〈<?= getEra($DownloadProgramYear-1) ?>申告用〉がインストールされた状態にしてください。</p>
                    <p>昨年のバージョンは<?= getEra($DownloadProgramYear) ?>の申告が終わってからアンインストールしてください。</p><br>

                    <h3><?= getEra($DownloadProgramYear) ?>申告用では、最新の税制改正や帳票様式の改訂に対応しています</h3>
                    <p>※くわしくは [ <b><a href="https://www.nta.go.jp/taxes/shiraberu/shinkoku/tokushu/kaisei.htm" target="_blank">国税庁ホームページ</a></b> ] をご覧ください。</p>

                    <h4>■各種申告書類等の様式変更</h4>
                    <ul>
                        <li>・確定申告書の押印が不要になりました。</li>
                        <li>・申告書第一表の「収入金額等」に「区分」欄が追加されました。</li>
                        <li>・住民税欄に「特定配当等の全部の申告不要」欄が追加されました。</li>
                        <li>・住宅借入金等特別控除の計算明細書に「契約日」や「不動産番号」の入力欄などが追加されました。</li>
                        <li>・その他、添付書類等の様式変更に対応しました。
                    </ul>
                        
                    <h4>■加算税（追徴金）の通知</h4>
                    <p>加算税（追徴金）が発生した場合、その通知をe-Taxで受けられるようになりました。</p>
                    ...
                    <h4>◆ダウンロードとインストール</h4>
                    <ol>
                        <li>下の「ダウンロード」ボタンをクリックし、「WeKakutei<?= (int)$DownloadProgramYear+1 ?>.exe」を任意のフォルダに保存してください。</li>
                        <li>ダウンロードした「WeKakutei<?= (int)$DownloadProgramYear+1 ?>.exe」をダブルクリックすると、ファイルの解凍先（初期値はC:\WeKakutei<?= (int)$DownloadProgramYear+1 ?>Setup）を指定する画面が表示されますので指示に従って解凍してください。解凍が完了するとインストールのメニュー画面が表示されます。</li>
                        <li>「みんなの確定申告」のボタンをクリックすると、インストールがはじまります。以降は、画面の指示に従ってインストールを行なってください。</li>
                        <li>インストール終了後、正常に動作させるためWindowsを再起動してください。</li>
                    </ol>
                    <p class="text-bold text-s">PDFマニュアルを閲覧するには、Adobe Reader が必要です。すでに Adobe Reader がインストールされている場合には、インストールする必要はありません。Adobe Reader をインストールする場合には、「WeKakutei<?= (int)$DownloadProgramYear+1 ?>.exe」の解凍先フォルダ（通常は C:\WeKakutei<?= (int)$DownloadProgramYear+1 ?>Setup）の中にある「Setup.exe」をダブルクリックすると、メニュー画面が表示されますので、「Adobe Reader」のボタンをクリックしてください。</p>
                    <p class="text-bold text-s">インストール後、ダウンロードした「WeKakutei<?= (int)$DownloadProgramYear+1 ?>.exe」と、（２）で解凍したフォルダのすべては削除して構いません。</p>
                    <h4>◆開始方法</h4>
                    ...
                    <p id="output-list">
                        <dl class="output">
                            ...
                            <dt class="output">・(付表)連帯債務がある場合の住宅借入金等の年末残高の計算明細書</dt>
                            <dt class="output">・住宅耐震改修特別控除額・住宅特定改修特別税額控除額の計算明細書</dt>
                    ...
                    <h3 id="downloadbox">みんなの確定申告〈<?= getEra($DownloadProgramYear) ?>申告用〉<br> <?= str_replace("-", ".", $DownloadProgramVersion) ?> インストーラー</h3>
                    <p>ファイルサイズ： 29.96 MB</p>
                    ...
                    <p style="margin-top:5px; color:#444;"><span style="color:#d00; font-weight:bold">【重要】 最新版サービスパックのご案内 [ 2022/2/8現在 ]</span><br>みんなの確定申告〈令和3年分申告用〉をご利用の際には、必ず<a href="https://member.sorimachi.co.jp/download/kakutei2021sp/?from=sorizo" target="_blank"><b>最新版のサービスパック</b></a>をダウンロードし、インストールしてからご入力ください。<br>最新版のサービスパックは<b>「みんなの確定申告」のオンラインアップデート機能</b>からもインストールすることができます。</p>
