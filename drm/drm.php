<?php
    ...
    $ListMenu = $ViewMenu = $ListWarn = $ViewWarn = '';
    switch ($Menu) {
        case "AddNew2":
            $ViewMenu = htmlspecialchars( @$_REQUEST["ViewMenu"] );
            $ViewWarn = htmlspecialchars( @$_REQUEST["ViewWarn"] );
            break;
...
//                $temp_result = mysqli_query($Conn, CheckSQL($sql));
...
// 2020/01/07 t.maruyama 追加 ↑↑
        ...
        case "conf2":
            if ( htmlspecialchars( @$_REQUEST["ConfMenu"] ) == "規定値に戻す" ) {
                ...
                UpdateCookie(CookieConf, $CookieArr, CookiesExpires);
                mysqli_free_result($result);
            }
            else {
...
<?php include($_SERVER['DOCUMENT_ROOT'] . '/lib/header_gtag_ga4.php'); ?>
        ...
        <FRAMESET ROWS="85, *">
            <FRAME SRC="<?= TopFile ?>?GroupID=<?= $GroupID ?>" NAME="MAINTOP" MARGINWIDTH="5" MARGINHEIGHT="5" SCROLLING="AUTO">
            <FRAMESET COLS="25%,*">
                <FRAME SRC="<?= GroupFile ?>?GroupID=<?= $GroupID ?>" NAME="GROUP" MARGINWIDTH="5" MARGINHEIGHT="5" SCROLLING="AUTO">
                <FRAMESET ROWS="40%,*">
                    <FRAME SRC="<?= ListFile ?>?Menu=<?= $ListMenu ?>&GroupID=<?= $GroupID ?>&Warn=<?= $ListWarn ?>" NAME="LIST" MARGINWIDTH="5" MARGINHEIGHT="5" SCROLLING="AUTO">
                    <FRAME SRC="<?= ViewFile ?>?Menu=<?= $ViewMenu ?>&GroupID=<?= $GroupID ?>&MesID=<?= $MesID ?>&Warn=<?= $ViewWarn ?>" NAME="VIEW" MARGINWIDTH="5" MARGINHEIGHT="5" SCROLLING="AUTO">
                </FRAMESET>
            </FRAMESET>
        </FRAMESET>
